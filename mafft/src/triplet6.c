...
void seq_grp_20( int *grp, char *seq )
{
	int tmp;
	while( *seq )
	{
...
int multipletripet( short *table, int *pointt )
{
	int value = 0;
	short tmp;
	int point;
...
void makepointtable_20( int *pointt, int *n )
{
	int point;
	register int *p;
...
int main( int argc, char **argv )
{
	...
	for( i=0; i<njob; i++ ) 
	{
		...
		{
			seq_grp_20( grpseq, tmpseq );
			makepointtable_20( pointt[i], grpseq );
		}
	}
	for( i=0; i<njob; i++ ) 
	{
		...
		for( j=i; j<njob; j++ ) 
		{
			score = (double)multipletripet( table1, pointt[j] );
			mtx[i][j] = score;
		} 
		free( table1 );
	}
